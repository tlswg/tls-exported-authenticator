...
Security (TLS) connection to its peer using a CertificateVerify message after the session
...
SHOULD use a secure protocol with equivalent security to TLS, such as QUIC {{QUIC-TLS}}, as its as its
...
generated by the client.)  These messages
...
for the pseudorandom function (PRF) (for TLS 1.2).  Exported authenticators cannot be
used with TLS 1.2 cipher suites that do not use the TLS PRF and with TLS 1.3 cipher suites that do not have an associated
...
Certificate message MUST contain only extensions present in the TLS ClientHello.
